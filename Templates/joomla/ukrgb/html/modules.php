...
/**
 * ukrgbDivision chrome.
 *
 * @since	1.6
 */
function modChrome_ukrgbDivision($module, &$params, &$attribs)
{
 ...
 * @since	1.6
 */
function modChrome_ukrgbHide($module, &$params, &$attribs)
{
	$headerLevel = isset($attribs['headerLevel']) ? (int) $attribs['headerLevel'] : 3;
...
<h<?php echo $headerLevel; ?> class="js_heading"><span class="backh"> <span
	...
	title="<?php echo JText::_('TPL_UKRGB_CLICK'); ?>"
	onclick="auf('module_<?php echo $module->id; ?>'); return false"
	class="opencloselink" id="link_<?php echo $module->id?>"> <span
	class="no"><img src="templates/ukrgb/images/plus.png"
	alt="<?php if ($state == 1) { echo JText::_('TPL_UKRGB_ALTOPEN');} else {echo JText::_('TPL_UKRGB_ALTCLOSE');} ?>" />
 ...
 * @since	1.6
 */
function modChrome_ukrgbTabs($module, $params, $attribs)
{
	...
	if ($modulecount == 1) {
		...
		foreach($modules as $rendermodule) {
			...
			{
			echo '<a href="#" class="unseen" onclick="nexttab(\'module_'. $rendermodule->id.'\');return false;" id="next_'.$rendermodule->id.'">'.JText::_('TPL_UKRGB_NEXTTAB').'</a>';
			}
			echo '</div>';
		}
		$modulecount--;
